...
private fun WidgetContent(active: Boolean, netMillis: Long) {
    Column(
        modifier = GlanceModifier.fillMaxSize().background(ColorProvider(R.color.widget_background)).padding(16.dp),
        verticalAlignment = Alignment.Vertical.CenterVertically
    ) {
        Text(text = if (active) "Working now" else "Not working", style = TextStyle(color = ColorProvider(if (active) R.color.widget_primary else R.color.widget_muted), fontWeight = FontWeight.Bold))
        Spacer(GlanceModifier.height(6.dp))
        Text(text = if (active) TimeUtils.formatDuration(netMillis) else "00:00", style = TextStyle(color = ColorProvider(R.color.widget_text), fontWeight = FontWeight.Bold))
        Spacer(GlanceModifier.height(10.dp))
        Row(modifier = GlanceModifier.fillMaxWidth()) {
            Text(text = if (active) "Tap to clock out" else "Tap to clock in", modifier = GlanceModifier.fillMaxWidth().clickable(actionRunCallback<ToggleShiftWidgetAction>()), style = TextStyle(color = ColorProvider(R.color.widget_primary)))
        }
    }
}
...
class ToggleShiftWidgetAction : ActionCallback {
    override suspend fun onAction(context: Context, glanceId: GlanceId, parameters: ActionParameters) {
        ...
        if (repository.getActive() != null) {
            ContextCompat.startForegroundService(context, Intent(context, ActiveShiftService::class.java).setAction(ActiveShiftService.ACTION_REFRESH))
        }
        WorkTrackerWidget().updateAll(context)
    }
}
