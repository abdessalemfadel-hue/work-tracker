...
class ReminderWorker(
    ...
    override suspend fun doWork(): Result {
        ...
        val notification = when {
            net >= target + 2 * 3_600_000L -> entry.notificationFactory().reminder(
                "Long shift detected",
                "You have worked ${TimeUtils.formatDuration(net)}. Consider clocking out or adding a note."
            )
            net >= target -> entry.notificationFactory().reminder(
                "Daily target reached",
                "You completed ${TimeUtils.formatDuration(target)} of net work time."
            )
            else -> null
        }
